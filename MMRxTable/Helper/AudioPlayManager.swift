...
class AudioPlayManager: NSObject {
    ...
    func startPlaying(_ audioModel: AudioModel) {
...
//                log.error("delegate is nil")
    ...
    // 转换，并且播放声音
    fileprivate func convertAmrToWavAndPlaySound(_ audioModel: AudioModel) {
        if self.audioPlayer != nil {
            self.stopPlayer()
        }
     ...
     */
    fileprivate func downloadAudio(_ audioModel: AudioModel) {
